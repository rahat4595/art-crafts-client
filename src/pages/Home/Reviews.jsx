

const Reviews = () => {
    return (
        <div>
            <section className="my-8  text-gray-100">

                <div className="container flex flex-col items-center justify-center mx-auto lg:flex-row lg:flex-wrap lg:justify-evenly ">
                    <div className="flex flex-col max-w-sm mx-4 my-6 overflow-hidden rounded-lg shadow-lg transition-transform duration-300 transform hover:-translate-y-2 hover:shadow-xl">
                        <div className="px-4 py-12 bg-gray-900 rounded-t-lg">
                            <p className="relative px-6 py-1 text-lg italic text-center text-gray-100">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" className="w-8 h-8 text-gray-200">
                                    <path d="M232,246.857V16H16V416H54.4ZM48,48H200V233.143L48,377.905Z"></path>
                                    <path d="M280,416h38.4L496,246.857V16H280ZM312,48H464V233.143L312,377.905Z"></path>
                                </svg>
                                This arts and crafts page offers a delightful blend of creativity and inspiration, making it a haven for enthusiasts seeking to explore their artistic side.
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" className="absolute right-0 w-8 h-8 text-gray-200">
                                    <path d="M280,185.143V416H496V16H457.6ZM464,384H312V198.857L464,54.1Z"></path>
                                    <path d="M232,16H193.6L16,185.143V416H232ZM200,384H48V198.857L200,54.1Z"></path>
                                </svg>
                            </p>
                        </div>
                        <div className="flex flex-col items-center justify-center p-8 bg-gray-200 text-gray-900">
                            <img src="https://i.ibb.co/NVFsMp0/pexels-justin-shaifer-501272-1222271.jpg" alt="" className="w-16 h-16 mb-2 -mt-16 rounded-full bg-gray-500 object-cover" />
                            <p className="text-xl font-semibold leading-tight">Justin Shaifer</p>
                            <p className="text-sm uppercase">Production Worker</p>
                        </div>
                    </div>

                    <div className="flex flex-col max-w-sm mx-4 my-6 overflow-hidden rounded-lg shadow-lg transition-transform duration-300 transform hover:-translate-y-2 hover:shadow-xl">
                        <div className="px-4 py-12 bg-gray-900 rounded-t-lg">
                            <p className="relative px-6 py-1 text-lg italic text-center text-gray-100">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" className="w-8 h-8 text-gray-200">
                                    <path d="M232,246.857V16H16V416H54.4ZM48,48H200V233.143L48,377.905Z"></path>
                                    <path d="M280,416h38.4L496,246.857V16H280ZM312,48H464V233.143L312,377.905Z"></path>
                                </svg>
                                Discover a captivating mix of creativity and inspiration of arts and crafts. And also invite other enthusiasts to indulge in their passion for artistic exploration and inspiration.
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" className="absolute right-0 w-8 h-8 text-gray-200">
                                    <path d="M280,185.143V416H496V16H457.6ZM464,384H312V198.857L464,54.1Z"></path>
                                    <path d="M232,16H193.6L16,185.143V416H232ZM200,384H48V198.857L200,54.1Z"></path>
                                </svg>
                            </p>
                        </div>
                        <div className="flex flex-col items-center justify-center p-8 bg-gray-200 text-gray-900">
                            <img src="https://i.ibb.co/dQCYKbx/pexels-elletakesphotos-1680172.jpg" alt="" className="w-16 h-16 mb-2 -mt-16 rounded-full bg-gray-500 object-cover" />
                            <p className="text-xl font-semibold leading-tight">Elle Hughes</p>
                            <p className="text-sm uppercase">NGO Worker</p>
                        </div>
                    </div>



                    <div className="flex flex-col max-w-sm mx-4 my-6 overflow-hidden rounded-lg shadow-lg transition-transform duration-300 transform hover:-translate-y-2 hover:shadow-xl">
                        <div className="px-4 py-12 bg-gray-900 rounded-t-lg">
                            <p className="relative px-6 py-1 text-lg italic text-center text-gray-100">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" className="w-8 h-8 text-gray-200">
                                    <path d="M232,246.857V16H16V416H54.4ZM48,48H200V233.143L48,377.905Z"></path>
                                    <path d="M280,416h38.4L496,246.857V16H280ZM312,48H464V233.143L312,377.905Z"></path>
                                </svg>
                                An inspiring hub for all things artistic! With a diverse array of crafts and a user-friendly interface. you'll find endless inspiration here. Explore more and be amazed
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" className="absolute right-0 w-8 h-8 text-gray-200">
                                    <path d="M280,185.143V416H496V16H457.6ZM464,384H312V198.857L464,54.1Z"></path>
                                    <path d="M232,16H193.6L16,185.143V416H232ZM200,384H48V198.857L200,54.1Z"></path>
                                </svg>
                            </p>
                        </div>
                        <div className="flex flex-col items-center justify-center p-8 bg-gray-200 text-gray-900">
                            <img src="https://i.ibb.co/CQLC6r6/pexels-brett-sayles-1073097.jpg" alt="" className="w-16 h-16 mb-2 -mt-16 rounded-full bg-gray-500 object-cover" />
                            <p className="text-xl font-semibold leading-tight">Brett Sayles</p>
                            <p className="text-sm uppercase">Entrepreneur</p>
                        </div>
                    </div>

                </div>
            </section>

            
        </div>
    );
};

export default Reviews;